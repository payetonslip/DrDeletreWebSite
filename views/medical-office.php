<h1>Le cabinet</h1>
<article class="presa-office">
    <p>
        Médecin généraliste depuis plusieurs années, le Docteur DELETRE Nicolas vous accueille à son cabinet pour des consultations d’alopécie (chute des cheveux), de médecine régénérative et de médecine esthétique, morphologique et anti-âge.

        La prise de rendez-vous s’effectue par Doctolib, et tout acte nécessite une consultation initiale avant programmation de soins à visée esthétique.

        Il vous reçoit :

        5 avenue de Lapeyrière, 31240 Saint Jean.

        En téléconsultation dédiée à ces actes si nécessaire.
    </p>
    <img src="../img/medical-office2.jpg" alt="">
</article>
<h2>Son parcours</h2>
<article class="formation">
    <img src="../img/dr-nicolas-deletre.jpg" alt="">

    <div>
        <section>
            <h3>ESTHETIQUE:</h3>
            <ul>
                <li>- Formation initiale du Collège International de Médecine Esthétique.
                    <br><br>2019-2020
                    <br><br>2020-2021
                </li>
                <li>- Formation approfondie en implantologie faciale à visée esthétique, C.I.M.E
                <br><br>2020-2021</li>
            </ul>
        </section>
        <section>
            <h3>FORMATION CONTINUE:</h3>
            <ul>
            <li>- Participation annuelle au congrès DEFEE (Dermatologues Fédérés Experts en Esthétique)</li>
            <li>- Participation annuelle au congrès France Médecine Esthétique</li>
            <li>- Participation au congrès 2024 AMWC Monaco (Aesthetic & Anti-Aging Medicine World Congress)</li>
            </ul>
        </section>
    </div>
</article>